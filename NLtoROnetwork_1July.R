...
data <- read_excel("NLtoROfinal.xlsx")
...
edge_data <- data.frame(
  ...
  Language = c(rep("Dutch", nrow(data)), rep("Romanian", nrow(data)), rep("Romanian", nrow(data)), rep("Dutch", nrow(data))),
  stringsAsFactors = FALSE
)
...
if (any(data$second_translator != "")) {
  second_translator_edges <- data.frame(
    ...
    Language = rep("Romanian", sum(data$second_translator != ""))
  )
  edge_data <- rbind(edge_data, second_translator_edges)
}

# Write nodes and edges to CSV files with correct formatting
write.csv(node_data, "NLtoROnodes.csv", row.names = FALSE, quote = TRUE, fileEncoding = "UTF-8")
write.csv(edge_data, "NLtoROedges.csv", row.names = FALSE, quote = TRUE, fileEncoding = "UTF-8")
